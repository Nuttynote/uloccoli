...
module.exports = async (req, res) => {
  ...
  const { password, id, song } = req.body || {};
  ...
  try {
    if (req.method === "POST") {
      ...
      const r = await fetch(`${SUPABASE_URL}/rest/v1/songs`, {
        method: "POST",
        headers: sbHeaders(),
    ...
    if (req.method === "PATCH") {
      ...
      const r = await fetch(`${SUPABASE_URL}/rest/v1/songs?id=eq.${encodeURIComponent(id)}`, {
        method: "PATCH",
        headers: sbHeaders(),
        body: JSON.stringify(body),
      });
